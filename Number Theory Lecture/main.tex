...
\pf{
	...
	\textbf{(1) If $a \equiv b \pmod{m}$, then $a \mod m = b \mod m$:}
	...
	\textbf{(2) If $a \mod m = b \mod m$, then $a \equiv b \pmod{m}$:}

	Assume $a \mod m = b \mod m$. This means that both $a$ and $b$ leave the same remainder when divided by $m$. Therefore, we can write:
	\[
		a = q_a \cdot m + r,
	\]
	and
	...
	Hence, we have shown both directions of the equivalence, completing the proof.
}
...
\pf{
	By the previous lemma, we know that if $a \equiv b \pmod{m}$, then there exists an integer $k_1$ such that $a = b + k_1 m$. Similarly, if $c \equiv d \pmod{m}$, then there exists an integer $k_2$ such that $c = d + k_2 m$.
	Now, we can substitute these expressions into the left-hand side of the congruences we want to prove.
	\[
	\begin{aligned}
		a + c & = (b + k_1 m) + (d + k_2 m) \\
			  & = (b + d) + (k_1 + k_2) m   \\
	...
	\[
	\begin{aligned}
		a \cdot c & = (b + k_1 m)(d + k_2 m)               \\
				  & = bd + (b k_2 + d k_1) m + k_1 k_2 m^2 \\
				  & \equiv bd \pmod{m}.
	\end{aligned}
	\]
...
\pf{
	We can prove these using similar strategies as in the previous theorem.
	\begin{enumerate}
		\item Assume \( a \equiv b \pmod{m} \). By definition, this means that \( m \mid (a - b) \), i.e., there exists an integer \( k_1 \) such that:
		      \[
			      a - b = k_1 m.
		      \]
		...
		\item The proof for subtraction follows similarly. If \( a \equiv b \pmod{m} \), then:
		      \[
			      a - c - (b - c) = (a - b) + 2c = k_1 m + 2c,
		      \]
		      which implies \( m \mid ((a - c) - (b - c)) \). Thus, \( a - c \equiv b - c \pmod{m} \).

		\item For multiplication, if \( a \equiv b \pmod{m} \), then:
		      \[
			      a \cdot c - b \cdot c = (a - b)c = k_1 m c,
		      \]
		      which implies \( m \mid (a \cdot c - b \cdot c) \). Thus, \( a \cdot c \equiv b \cdot c \pmod{m} \).
	\end{enumerate}
	Therefore, all three properties are proved.
}
...
\pf{
	We want to prove that the set of congruence classes modulo \( m \)
	\[
	...
	\begin{enumerate}
		\item Non-emptiness: For each \( r \in \{0,1,\dots,m-1\} \), the set \( [r]_m \) is non-empty.
		\item Pairwise disjointness: If \( a,b \in \{0,1,\dots,m-1\} \) and \( a \neq b \), then \( [a]_m \cap [b]_m = \emptyset \).
		\item Union covers the set: \( \bigcup_{r=0}^{m-1} [r]_m = \mathbb{Z} \).
	\end{enumerate}
	\vspace{6pt}
	\noindent
	\textbf{1. Non-emptiness:}
	For any \( r \in \{0,1,\dots,m-1\} \), it is clear that \( r \in [r]_m \), so \( [r]_m \neq \emptyset \).

	\vspace{6pt}
	\noindent
	\textbf{2. Pairwise disjointness:}
	Assume there exist \( a,b \in \{0,1,\dots,m-1\} \), with \( a \neq b \), such that \( [a]_m \cap [b]_m \neq \emptyset \).
	...
	\vspace{6pt}
	\noindent
	\textbf{3. Union covers the set:}
	For any \( n \in \mathbb{Z} \), by the division algorithm, there exist unique integers \( q \in \mathbb{Z} \) and \( r \in \{0,1,\dots,m-1\} \) such that
	\[
		n = qm + r.
	\]
	...
	\]
	In summary, the set of congruence classes modulo \( m \) forms a partition of the set of integers \( \mathbb{Z} \).
}
...
\pf{
To show that the addition and multiplication operations on the residue classes modulo \( m \) are well-defined, we need to prove that if \( [a]_m = [a']_m \) and \( [b]_m = [b']_m \), then:
\[
	[a]_m + [b]_m = [a' + b']_m,
\]
and
...
\]
Both terms on the right-hand side are multiples of \( m \), so we
can conclude that:
\[
...
\[
	[a]_m \cdot [b]_m = [a \cdot b]_m.
\]
Therefore, the addition and multiplication operations on the residue classes modulo \( m \) are well-defined.
}
...
\pf{
	To show that \( G \) is an abelian group, we need to prove that \( xy = yx \) for all \( x, y \in G \).
...
\pf{
	By peano axioms, 0 is not a successor of any natural number, and addition on natural numbers is defined recursively as follows:
	\begin{itemize}
		\item \( a + 0 = a \)
		\item \( a + S(b) = S(a + b) \)
	\end{itemize}
...
\pf{
By previous resultsm we know that
$$\gcd(a, b) = p_1^{\min(a_1, b_1)} p_2^{\min(a_2, b_2)} \ldots p_n^{\min(a_n, b_n)},$$
$$\text{lcm}(a, b) = p_1^{\max(a_1, b_1)} p_2^{\max(a_2, b_2)} \ldots p_n^{\max(a_n, b_n)}.$$
Then we have
$$\gcd(a, b) \cdot \text{lcm}(a, b) = p_1^{\min(a_1, b_1)} p_2^{\min(a_2, b_2)} \ldots p_n^{\min(a_n, b_n)} \cdot p_1^{\max(a_1, b_1)} p_2^{\max(a_2, b_2)} \ldots p_n^{\max(a_n, b_n)}$$
We can combine the two products:
$$\gcd(a, b) \cdot \text{lcm}(a, b) = p_1^{\min(a_1, b_1) + \max(a_1, b_1)} p_2^{\min(a_2, b_2) + \max(a_2, b_2)} \ldots p_n^{\min(a_n, b_n) + \max(a_n, b_n)}$$
We know that $\min(a_i, b_i) + \max(a_i, b_i) = a_i + b_i$ for all $i$. Thus we have
$$\gcd(a, b) \cdot \text{lcm}(a, b) = p_1^{a_1 + b_1} p_2^{a_2 + b_2} \ldots p_n^{a_n + b_n} = ab.$$
This completes the proof.
}
...
\pf{
	For $a, b \in \mathbb{Z}$, $a \geq b > 0$, compute the gcd via:
	$$
		\begin{aligned}
	...
	$$
		r_k = a x_k + b y_k \quad \text{for } k = 0, 1, \dots, n.
	$$
	\textbf{Base Cases:}
	$$
		\begin{cases}
			r_0 = a = a \cdot 1 + b \cdot 0 \implies x_0 = 1, y_0 = 0, \\
			r_1 = b = a \cdot 0 + b \cdot 1 \implies x_1 = 0, y_1 = 1.
		\end{cases}
	$$
	\textbf{Inductive Step:}
	Assume for some $k \geq 1$:
	$$
		r_{k-1} = a x_{k-1} + b y_{k-1}, \quad r_k = a x_k + b y_k.
	$$
	From the division step $r_{k+1} = r_{k-1} - q_{k+1} r_k$, substitute the expressions for $r_{k-1}$ and $r_k$:
	$$
		r_{k+1} = \left(a x_{k-1} + b y_{k-1}\right) - q_{k+1} \left(a x_k + b y_k\right).
	$$
	Group terms:
	$$
		r_{k+1} = a \left(x_{k-1} - q_{k+1} x_k\right) + b \left(y_{k-1} - q_{k+1} y_k\right).
	$$
	Define:
	$$
		x_{k+1} = x_{k-1} - q_{k+1} x_k, \quad y_{k+1} = y_{k-1} - q_{k+1} y_k.
	$$
	Thus, $r_{k+1}$ is also a linear combination of $a$ and $b$.
	After $n$ steps, the gcd $r_n$ satisfies:
	$$
		r_n = a x_n + b y_n,
	$$
	where $x_n, y_n$ are integers derived recursively. Hence, $\gcd(a, b)$ is expressible as $ax + by$ for some $x, y \in \mathbb{Z}$.
}
...
\defn{Extended Euclidean Algorithm}{
